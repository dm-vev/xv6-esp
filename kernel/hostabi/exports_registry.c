...
static int rebuild_locked(void)
{
  ...
  for(i = 0; i < n; i++){
    elf_host_symbol_t hs;
    hs.name = g_mod_syms[idx[i]].name;
    hs.addr = g_mod_syms[idx[i]].addr;
    if(elf_loader_register_host_symbols(&hs, 1) != 0)
      return -1;
  }
...
int hostabi_export_add_module(const hostabi_module_symbol_t *syms, int count)
{
  ...
  if(i != count || rebuild_locked() != 0){
    int j;
    for(j = 0; j < HOSTABI_MODSYM_MAX; j++){
      if(g_mod_syms[j].used && g_mod_syms[j].seq >= start_seq)
        memset(&g_mod_syms[j], 0, sizeof(g_mod_syms[j]));
    }
    (void)rebuild_locked();
    exports_unlock();
    return -1;
